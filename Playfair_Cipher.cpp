...
signed main(){
    ...
    for(int i = 0; i < ENCODED_MESSAGE.size(); i+=2){
        ...
        if(row1 == row2){
            decoded_message += cipher_square[row1][((col1 - 1)%5 + 5)%5];
            decoded_message += cipher_square[row2][((col2 - 1)%5 + 5)%5];
        }


        //Taking the letters to the left of each letter if they are in the same column(with a wrap around)
        else if(col1 == col2){
            decoded_message += cipher_square[((row1 - 1)%5 + 5)%5][col1];
            decoded_message += cipher_square[((row2 - 1)%5 + 5)%5][col2];
        }
